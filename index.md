...
[`annotree()`](https://jtr13.github.io/annotreer/reference/annotree.md),
which is a wrapper for
[`rpart.plot::prp()`](https://rdrr.io/pkg/rpart.plot/man/prp.html). By
employing customized labeling functions under the hood,
[`annotree()`](https://jtr13.github.io/annotreer/reference/annotree.md)
helps us visualize complexity parameter calculations. These annotations
are helpful for explaining CART’s pruning algorithm.
...
[`rpart.plot::prp()`](https://rdrr.io/pkg/rpart.plot/man/prp.html) to
create tree diagrams that will aid in understanding how this table
summarizes the pruning options. Let’s start with an example.

We create a large tree with
[`rpart::rpart()`](https://rdrr.io/pkg/rpart/man/rpart.html) and
visualize it with
[`annotree()`](https://jtr13.github.io/annotreer/reference/annotree.md).
The data is from the **rpart** package:
...
to
[`annotree()`](https://jtr13.github.io/annotreer/reference/annotree.md)
below, the nodes are labeled with class labels and class counts.
...
Since we set the parameters of
[`rpart()`](https://rdrr.io/pkg/rpart/man/rpart.html) to grow as large a
tree as possible, the deviance for all of the leaf nodes is zero: every
leaf node has a zero for one of the classes. We can make the deviance of
the leaves more explict by setting `show_leaf_dev` to `TRUE`:
...
Now we turn to the pruning process.
[`rpart()`](https://rdrr.io/pkg/rpart/man/rpart.html) returns a
`cptable` which provides pruning options. Following the cost complexity
pruning algorithm of CART (see Breiman, et al. 1984) the table does not
show every possible subtree, only the ones produced by sequentially
pruning the next weakest link until the tree is reduced to a single
node.
...
To understand how [`rpart()`](https://rdrr.io/pkg/rpart/man/rpart.html)
is created we’ll start with some definitions:
...
$g(t) = \frac{R(t) - R\left( T_{t} \right)}{\left| {\widetilde{T}}_{t} \right| - 1},$
where

- $t$ is a node in the decision tree,

- $T_{t}$ is the subtree rooted at node $t$, including $t$ and all of
  its descendants,

- ${\widetilde{T}}_{t}$ is the set of terminal (leaf) nodes in the
  subtree $T_{t}$,

- $R(t)$ is the risk at node $t$, in this case, the deviance or
  misclassified cases,

- $R\left( T_{t} \right)$ is the total risk of the subtree $T_{t}$,
  meaning the sum of risks at all terminal nodes in $T_{t}$, and

- $\left| {\widetilde{T}}_{t} \right|$ is the number of terminal nodes
  (leaves) in the subtree $T_{t}$,.
...
![](reference/figures/README-unnamed-chunk-11-1.svg)
...
![](reference/figures/README-unnamed-chunk-13-1.svg)
